...
import { TextComponent, TextColour, TextSize, TextFont } from "../components/TextComponent";
...
export const InventoryContainer: React.FunctionComponent<InventoryContainerProps> = props => {
    ...
    useDispatchListener((action: Actions) => {
        switch (action.type) {
            ...
            case PlayerEventType.PLAYER_INVENTORY_CLOSED:
                animation(x => setFade(1 - x)).speed(FADE_IN).driven().start().whenDone(
    ...
    return (
        <ViewportComponent
            ...
            {
            inventoryShowing && <TextComponent 
                text={"Inventory"}
